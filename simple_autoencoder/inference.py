...
def main(argv):
    ...
    for subject in subjects:
        ...
        model_config = load_model_config(f"{PROJECT_DIR}/results/subj{subject}/{user_provided_args.config.model_config}/config")
        log('loaded model config', 'ANALYSIS')
        print(model_config)

        # load the analysis data
        # ndr: the images are the same, but I need subject to access the right fmri indices
        person, non_person = get_analysis_datasets('person', subject, roi_class=model_config['roi_class'], hem=model_config['hem'])
